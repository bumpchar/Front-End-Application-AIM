...
# We had 4 different collections that we tested
# NBA_Data_Whole_Chunker -> takes each txt document and chunks it indiviudally
# NBA_Data_Semanitc -> Semantic Chunker using llm
# NBA_Data_Sentence_5 -> Sentence chunker with a window size of 5
# NBA_Data_CharLen -> uses CharLen Chunker with chunk size of 500 and overlap of 50
vdb.initialize_collection("NBA_Data_Semantic")

# Test RAG Agent
rag_agent = ChromaAgent(llm, vdb)
rag_agent.query("Which heat player had a career-high game on March 12, 2026? ", 
                k = 5, 
                max_distance = 0.65, 
                show_citations =True)
# Imports
...
db_url = "../data/FINA 4420 - NBA DB.db"
...
# Ask it questions
response = sqlite_agent.query("Which team has the highest average bench points per game in 2026?  ", view_sql=True)
print(response)
# Imports
import os
